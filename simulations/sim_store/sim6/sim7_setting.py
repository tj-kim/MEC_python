...
def simulation_setting_limited():    
    """
    ...
    weak_range = np.array([[6,6],[1000,1500],[5,5]])
    strong_range = np.array([[10,10],[10000,20000],[20,20]])
...
def simulation_setting_ample():    
    ...
    Make Simulation Parameters
    """
    sim_param = Sim_Params(time_steps=10, x_length = 2.4, y_length = 1.7, max_edge_length=2,num_path_limit=5)
    boundaries = np.array([[0,sim_param.x_length],[0,sim_param.y_length]])
    ...
    num_resource = 3
    # (cores, storage GB, ram)
    weak_range = np.array([[40,60],[1000,1500],[40,160]])
    strong_range = np.array([[100,200],[10000,20000],[1000,1500]])
    ...
    num_link = [0,1,2,3]
    prob_link = [0.5,0.2,0.2,0.1]
    lv_minmax = np.array(([[250,400],[1000,3000],[3000,5000]]))
    lv1_transmission = 1
    link_costs = np.array([0.06, 0.06, 0.06])
    latency_settings = [250 * 1e-3, 50 * 1e-3] #[ms per switch, ms per mile]
