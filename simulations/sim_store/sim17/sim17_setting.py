...
def simulation_setting(db, speed, var):    
    
    """
    Make Simulation Parameters
    """
    sim_param = Sim_Params(time_steps=10, x_length = 5, y_length = 5, max_edge_length=3,num_path_limit=5)
    ...
    # Custom locations 
    custom_locs = [[1,1],[1,2.5],[1,4],[4,1],[4,2.5],[4,4],[2.5,1],[2.5,4],[2.5,2.5]]
    ...
    lv1_transmission = 1
    link_costs = np.array([0.06, 0.06, 0.06]) # multiply cost by 2
    latency_settings = [250 * 1e-3, 50 * 1e-3] #[ms per switch, ms per mile] - div cost by 2
    ...
    spd_thresh = [speed-var, speed+var]
